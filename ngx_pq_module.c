...
static ngx_int_t ngx_pq_copy_handler(ngx_http_request_t *r) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    ngx_http_upstream_t *u = r->upstream;
    ngx_pq_data_t *d = u->peer.data;
    ngx_pq_save_t *s = d->save;
    ...
    switch ((len = PQgetCopyData(s->conn, &buffer, 0))) {
        case 0: break;
        case -1: break;
        case -2: ngx_pq_log_error(NGX_LOG_ERR, r->connection->log, 0, PQerrorMessageMy(s->conn), "PQgetCopyData == -2"); rc = NGX_HTTP_BAD_GATEWAY; break;
...
static ngx_int_t ngx_pq_error_handler(ngx_http_request_t *r) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "%s", __func__);
    ngx_http_upstream_t *u = r->upstream;
    ngx_pq_data_t *d = u->peer.data;
    ngx_pq_save_t *s = d->save;
...
/*        ngx_str_t id = { ngx_strlen(notify->relname), (u_char *)notify->relname };
        ...
        switch ((rc = ngx_http_push_stream_add_msg_to_channel_my(s->connection->log, &id, &text, NULL, NULL, 1, temp_pool))) {
            ...
            case NGX_DECLINED: ngx_log_error(NGX_LOG_WARN, s->connection->log, 0, "ngx_http_push_stream_add_msg_to_channel_my == NGX_DECLINED"); {
                ...
                if (!escape) { ngx_pq_log_error(NGX_LOG_ERR, s->connection->log, 0, PQerrorMessageMy(s->conn), "!PQescapeIdentifier(%V)", &id); break; }
...
/*    if (str.len) {
        if (!(str.data = ngx_pnalloc(s->connection->pool, str.len + 1))) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "!ngx_pnalloc"); rc = NGX_ERROR; } else {
            ...
            *p = '\0';
            if (!PQsendQuery(s->conn, (const char *)str.data)) { ngx_pq_log_error(NGX_LOG_ERR, s->connection->log, 0, PQerrorMessageMy(s->conn), "!PQsendQuery(\"%V\")", &str); rc = NGX_ERROR; } else {
...
static ngx_int_t ngx_pq_result_handler(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ...
    ngx_http_request_t *r = d->request;
    if (!PQconsumeInput(s->conn)) { ngx_pq_log_error(NGX_LOG_ERR, c->log, 0, PQerrorMessageMy(s->conn), "!PQconsumeInput"); return NGX_HTTP_BAD_GATEWAY; }
    ...
    while (PQstatus(s->conn) == CONNECTION_OK) {
        ...
        if (!ngx_queue_empty(&d->queue)) {
            q = ngx_queue_head(&d->queue);
            ngx_pq_query_queue_t *qq = ngx_queue_data(q, ngx_pq_query_queue_t, queue);
            d->query = qq->query;
        }
        switch (PQresultStatus(s->res)) {
            case PGRES_COMMAND_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "PGRES_COMMAND_OK"); if (rc == NGX_OK) rc = ngx_pq_command_handler(r); break;
            case PGRES_COPY_OUT: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "PGRES_COPY_OUT"); if (rc == NGX_OK && d->query->output.type) rc = ngx_pq_copy_handler(r); break;
            case PGRES_FATAL_ERROR: rc = ngx_pq_error_handler(r); break;
            case PGRES_PIPELINE_SYNC: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "PGRES_PIPELINE_SYNC"); PQclear(s->res); goto done;
            case PGRES_TUPLES_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "PGRES_TUPLES_OK"); if (rc == NGX_OK && d->query->output.type) rc = ngx_pq_tuple_handler(r); break;
            default: ngx_log_error(NGX_LOG_ERR, c->log, 0, "%s not supported", PQresStatus(PQresultStatus(s->res))); rc = NGX_HTTP_BAD_GATEWAY; q = ngx_queue_head(&d->queue); ngx_queue_remove(q); break;
        }
    ...
    if ((s->res = PQgetResult(s->conn))) { ngx_log_error(NGX_LOG_ERR, c->log, 0, "PQgetResult"); return NGX_HTTP_BAD_GATEWAY; }
    if (!PQexitPipelineMode(s->conn)) { ngx_pq_log_error(NGX_LOG_ERR, c->log, 0, PQerrorMessageMy(s->conn), "!PQexitPipelineMode"); return NGX_HTTP_BAD_GATEWAY; }
    if (!ngx_queue_empty(&d->queue)) { ngx_log_error(NGX_LOG_ERR, c->log, 0, "!ngx_queue_empty"); return NGX_HTTP_BAD_GATEWAY; }
    if (rc == NGX_OK && d->query->type & ngx_pq_type_upstream) return ngx_pq_queries(d, ngx_pq_type_location);
    return rc;
}
...
static ngx_int_t ngx_pq_connect_handler(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ngx_connection_t *c = s->connection;
    switch (PQstatus(s->conn)) {
        case CONNECTION_BAD: ngx_pq_log_error(NGX_LOG_ERR, c->log, 0, PQerrorMessageMy(s->conn), "CONNECTION_BAD"); return NGX_HTTP_BAD_GATEWAY;
        case CONNECTION_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "CONNECTION_OK"); return ngx_pq_queries(d, ngx_pq_type_location|ngx_pq_type_upstream);
        default: break;
    }
    switch (PQconnectPoll(s->conn)) {
        ...
        case PGRES_POLLING_FAILED: ngx_pq_log_error(NGX_LOG_ERR, c->log, 0, PQerrorMessageMy(s->conn), "PGRES_POLLING_FAILED"); return NGX_HTTP_BAD_GATEWAY;
        case PGRES_POLLING_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "PGRES_POLLING_OK"); return ngx_pq_queries(d, ngx_pq_type_location|ngx_pq_type_upstream);
...
static void ngx_pq_read_event_handler(ngx_http_request_t *r, ngx_http_upstream_t *u) {
    ...
    switch (PQstatus(s->conn)) {
        ...
        case CONNECTION_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "CONNECTION_OK"); return ngx_pq_upstream_finalize_request(r, u, ngx_pq_result_handler(s, d));
    ...
    return ngx_pq_upstream_finalize_request(r, u, ngx_pq_connect_handler(s, d));
}
