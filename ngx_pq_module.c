...
static ngx_int_t ngx_pq_res_command_ok(ngx_pq_save_t *s, ngx_pq_data_t *d, PGresult *res) {
    ...
    if ((value = PQcmdStatus(res)) && (len = ngx_strlen(value))) { ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "%s and %s", PQresStatus(PQresultStatus(res)), value); }
    else { ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "%s", PQresStatus(PQresultStatus(res))); }
    if (s->count) { s->count--; return NGX_OK; }
    if (!d) return NGX_OK;
    if (ngx_queue_empty(&d->queue)) { ngx_log_error(NGX_LOG_ERR, c->log, 0, "ngx_queue_empty"); return NGX_ERROR; }
...
static ngx_int_t ngx_pq_poll(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ngx_connection_t *c = s->connection;
    ngx_flag_t started = (PQstatus(s->conn) == CONNECTION_STARTED);
again:
...
static ngx_int_t ngx_pq_fail_poll(ngx_pq_fail_t *f) {
    ...
    ngx_flag_t started = (PQcancelStatus(f->conn) == CONNECTION_STARTED);
again:
...
#ifdef LIBPQ_HAS_ASYNC_CANCEL
static void ngx_pq_fail_cln_handler(void *data) {
...
static void ngx_pq_fail_handler(ngx_event_t *ev) {
    ...
    switch (PQcancelStatus(f->conn)) {
        ...
        default: ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "PQcancelStatus = %i", PQcancelStatus(f->conn)); break;
    }
...
static ngx_conf_enum_t ngx_pq_empty[] = {
