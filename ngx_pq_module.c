...
static ngx_int_t ngx_pq_output(ngx_pq_save_t *s, ngx_pq_data_t *d, const u_char *data, size_t len) {
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%*s", (int)len, data);
    if (!len) return NGX_OK;
    ngx_pq_query_t *query = s->query.cur;
...
static ngx_int_t ngx_pq_tuple(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ...
    if (!result->cur) return NGX_OK;
    ngx_pq_query_t *query = result->cur;
    if (query->output.header) {
        if (result->row > 0) if (ngx_pq_output(s, d, (const u_char *)"\n", sizeof("\n") - 1) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
        for (result->col = 0; result->col < PQnfields(result->res); result->col++) {
            if (result->col > 0) if (ngx_pq_output(s, d, &query->output.delimiter, sizeof(query->output.delimiter)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            if (query->output.string && query->output.quote) if (ngx_pq_output(s, d, &query->output.quote, sizeof(query->output.quote)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            const u_char *data = (const u_char *)PQfname(result->res, result->col);
            ngx_uint_t len = ngx_strlen(data);
            if (query->output.string && query->output.quote && query->output.escape) for (ngx_uint_t k = 0; k < len; k++) {
                if (data[k] == query->output.quote) if (ngx_pq_output(s, d, &query->output.escape, sizeof(query->output.escape)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
                if (ngx_pq_output(s, d, &data[k], sizeof(data[k])) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            } else {
                if (ngx_pq_output(s, d, (const u_char *)data, len) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            }
            if (query->output.string && query->output.quote) if (ngx_pq_output(s, d, &query->output.quote, sizeof(query->output.quote)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
    }
    for (result->row = 0; result->row < PQntuples(result->res); result->row++) {
        if (result->row > 0 || query->output.header) if (ngx_pq_output(s, d, (const u_char *)"\n", sizeof("\n") - 1) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
        for (result->col = 0; result->col < PQnfields(result->res); result->col++) {
            if (result->col > 0) if (ngx_pq_output(s, d, &query->output.delimiter, sizeof(query->output.delimiter)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            if (PQgetisnull(result->res, result->row, result->col)) {
                if (query->output.null.len) if (ngx_pq_output(s, d, query->output.null.data, query->output.null.len) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
            } else {
                if (query->output.string && query->output.quote) if (ngx_pq_output(s, d, &query->output.quote, sizeof(query->output.quote)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
                const u_char *data = (const u_char *)PQgetvalue(result->res, result->row, result->col);
                ngx_uint_t len = PQgetlength(result->res, result->row, result->col);
                if (query->output.string && query->output.quote && query->output.escape) for (ngx_uint_t k = 0; k < len; k++) {
                    if (data[k] == query->output.quote) if (ngx_pq_output(s, d, &query->output.escape, sizeof(query->output.escape)) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
                    if (ngx_pq_output(s, d, &data[k], sizeof(data[k])) != NGX_OK) return NGX_HTTP_INTERNAL_SERVER_ERROR;
                } else {
...
static ngx_int_t ngx_pq_copy(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ...
    if (!d || !s->query.cur) return NGX_OK;
    char *buffer = NULL;
    int len;
    ngx_int_t rc = NGX_OK;
    switch ((len = PQgetCopyData(s->conn, &buffer, 0))) {
        ...
        default: s->query.row++; if (ngx_pq_output(s, d, (const u_char *)buffer, len) != NGX_OK) rc = NGX_HTTP_INTERNAL_SERVER_ERROR; break;
    }
...
static ngx_int_t ngx_pq_default(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ...
    const char *value;
    if ((value = PQcmdStatus(s->query.res)) && ngx_strlen(value)) { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "%s and %s not supported", PQresStatus(PQresultStatus(s->query.res)), value); }
    else { ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "%s not supported", PQresStatus(PQresultStatus(s->query.res))); }
    return NGX_HTTP_BAD_GATEWAY;
}
...
static ngx_int_t ngx_pq_command(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ...
    const char *value;
    size_t len = 0;
    if ((value = PQcmdStatus(s->query.res)) && (len = ngx_strlen(value))) { ngx_log_debug2(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s and %s", PQresStatus(PQresultStatus(s->query.res)), value); }
    else { ngx_log_debug1(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "%s", PQresStatus(PQresultStatus(s->query.res))); }
    if (ngx_http_push_stream_delete_channel_my && d && s->query.cur && s->query.cur->commands.nelts == 2 && len == sizeof("LISTEN") - 1 && !ngx_strncasecmp(value, (u_char *)"LISTEN", sizeof("LISTEN") - 1)) {
        ngx_pq_command_t *command = s->query.cur->commands.elts;
        command = &command[1];
...
static ngx_int_t ngx_pq_queries(ngx_pq_data_t *d, ngx_uint_t type) {
    ...
    initPQExpBuffer(&sql);
    if (!PQenterPipelineMode(s->conn)) { ngx_pq_log_error(NGX_LOG_ERR, r->connection->log, 0, PQerrorMessageMy(s->conn), "!PQenterPipelineMode"); goto ret; }
    ...
    for (ngx_uint_t i = 0; i < queries->nelts; i++) {
        ...
        if (!(qq->paramTypes = ngx_pcalloc(r->pool, qq->nParams * sizeof(*qq->paramTypes)))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pcalloc"); goto ret; }
        if (!(qq->paramValues = ngx_pcalloc(r->pool, qq->nParams * sizeof(*qq->paramValues)))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pcalloc"); goto ret; }
        for (ngx_int_t j = 0; j < qq->nParams; j++) {
            if (query[i].type & ngx_pq_type_query || query[i].type & ngx_pq_type_prepare) {
                if (argument[j].oid.index) {
                    ...
                    if (n == NGX_ERROR) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_atoi == NGX_ERROR"); goto ret; }
            ...
            if (query[i].type & ngx_pq_type_query || query[i].type & ngx_pq_type_execute) {
                ...
                if (!(qq->paramValues[j] = ngx_pnalloc(r->pool, argument[j].value.str.len + 1))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_pnalloc"); goto ret; }
        ...
        for (ngx_uint_t j = 0; j < query[i].commands.nelts; j++) if (command[j].index) {
            char *str;
            ngx_http_variable_value_t *value;
            if (!(value = ngx_http_get_indexed_variable(r, command[j].index))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_http_get_indexed_variable"); goto ret; }
            if (!(str = PQescapeIdentifier(s->conn, (const char *)value->data, value->len))) { ngx_pq_log_error(NGX_LOG_ERR, r->connection->log, 0, PQerrorMessageMy(s->conn), "!PQescapeIdentifier"); goto ret; }
            appendPQExpBufferStr(&sql, str);
            PQfreemem(str);
        ...
        if (query[i].type & ngx_pq_type_query) {
            ...
            if (query[i].name.index) {
                ngx_http_variable_value_t *value;
                if (!(value = ngx_http_get_indexed_variable(r, query[i].name.index))) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "!ngx_http_get_indexed_variable"); goto ret; }
...
static ngx_int_t ngx_pq_connect(ngx_pq_save_t *s, ngx_pq_data_t *d) {
    ...
    switch (PQstatus(s->conn)) {
        ...
        case CONNECTION_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "CONNECTION_OK"); return ngx_pq_queries(d, ngx_pq_type_location|ngx_pq_type_upstream);
        default: break;
    }
    switch (PQconnectPoll(s->conn)) {
        ...
        case PGRES_POLLING_OK: ngx_log_debug0(NGX_LOG_DEBUG_HTTP, s->connection->log, 0, "PGRES_POLLING_OK"); return ngx_pq_queries(d, ngx_pq_type_location|ngx_pq_type_upstream);
